...
//
// Gated on cookie-consent.js (must be loaded first — see index.html): the
// PostHog snippet never loads at all, let alone captures anything, until the
// visitor has explicitly clicked "Accept" on the cookie banner.
//
...
(function () {
  ...
  if (window.QuizomaConsent) {
    ...
    window.QuizomaConsent.onRevoke(function () {
      try { if (window.posthog) posthog.opt_out_capturing(); } catch (e) {}
      window.Analytics = { identify: noop, reset: noop, track: noop, screen: noop };
    });
  }
  // If cookie-consent.js failed to load for some reason, window.Analytics stays
  // the safe noop stub above — never tracks without an explicit grant.
})();
